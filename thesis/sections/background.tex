  ...
  \label{sec:background}
    ...
    \label{sub:finite_elemente_methode}
      ...
      \subsubsection{Wahl der Basisfunktionen}
        ...
        \]
        Das Ziel wird es sein, jede beliebige Funktion durch Elemente dieses Raumes zu beschreiben.
        Um dies zu vereinfachen, soll $\mathrm{S}$ in drei verschiedene Unterräume separiert werden.
        $\mathrm{S}^\circ$ bezeichnet hierbei den Raum der Funktionen, die auf dem Rand $\boundary$ identisch zu Null sind.
        $\mathrm{S}^\mathrm{(D)}$
      ...
      \subsubsection{Konstruktion der algebraischen Formulierung}
        ...
        \hfill\\
          ...
          \[
            u
            = \sum_{i\in V} u_i η_i
            = \underbrace{\sum_{i\in V^\circ} u_i η_i}_{= v}
            + \underbrace{\sum_{i\in V_\mathrm{D}} u^\mathrm{D}_i η_i}_{=u^\mathrm{(D)}}
            \separate
            f = \sum_{i\in V} f_i η_i
            \separate
            v = \sum_{i\in V^\circ} v_i η_i
            \separate
            φ = \sum_{i\in V^\circ} φ_i η_i
          \]
          \[
            u^\mathrm{(D)} = \sum_{i\in V_\mathrm{D}} u^\mathrm{(D)}_i η_i
            \separate
            u^\mathrm{(N)} = \sum_{i\in V_\mathrm{N}} u^\mathrm{(N)}_i \appendValue{η_i}{\neumannBoundary}
          \]
          \[
            \integral{\domain}{}{\scalarProduct{\nabla v}{\nabla φ}}{λ} = \integral{\domain}{}{f φ}{λ} + \integral{\neumannBoundary}{}{u^\mathrm{(N)} φ}{σ} - \integral{\domain}{}{\scalarProduct{\nabla u^\mathrm{(D)}}{\nabla φ}}{λ}
          \]
          Weiterhin sei die Menge der Eckpunkte des diskretisierten Berechnungsgebietes gegeben durch $V$ und deren zugehörige Basisfunktionen durch die Menge $B\define\set{\function{η_i}{\domain}{\setReal}}{i\in V}$.
          Im diskretisierten Falle gilt demnach für alle $ξ\in\mathrm{S}_\mathrm{D}$ die folgende Gleichung für die diskretisierten Funktionen $v\in\mathrm{S}_\mathrm{D}$, $f\in\mathrm{S}$.
          \[
            \sum_{j\in V^\circ} v_j \integral{\domain}{}{\scalarProduct{\nabla η_i}{\nabla η_j}}{λ}
          ...
          \]
          Man ersetzt zunächst die Funktionen $φ\in \setSobolev^1_\mathrm{D}(\domain)$ durch Funktionen $ξ\in \mathrm{S}_\mathrm{D}$ des diskretisierten Raumes.
          Da es sich bei $B$ um eine Basis dieses Raumes handelt, lässt sich jedes $ξ\in\mathrm{S}_\mathrm{D}$ durch eine Linearkombination der Basisfunktionen beschreiben.
