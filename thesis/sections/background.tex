  ...
  \label{sec:background}
    ...
    \label{sub:finite_elemente_methode}
      ...
      \label{par:schritt_1_konstruktion_der_schwachen_formulierung}
        ...
        Auf die Poisson-Gleichung wurde dieses Verfahren bereits angewendet.
        Allerdings wurde in den Abschnitten \ref{ssub:wellengleichung} und \ref{ssub:heat-equation} erwähnt, dass die für die Finite-Elemente-Methode benötigte schwache Formulierung der Wellen- und Wärmeleitungsgleichung durch die separate Diskretisierung der Zeit erlangt wird.
        Es sei also ein kleiner Zeitschritt $\infinitesimal{t}\in (0,\infty)$ gegeben.
        Im Falle der Wärmeleitungsgleichung wendet man nun ein implizites Euler-Verfahren auf die zeitliche Ableitung an \cite{Alberty1998}.
        Dies stellt sicher, dass die Lösung des Problems nicht divergiert.
        \[
          \partial_t u \approx \frac{u_{n+1} - u_n}{\infinitesimal{t}} \approx \laplacian u_{n+1} + f_{n+1}
        \]
        \[
          \roundBrackets{\identity - \infinitesimal{t}\laplacian}u_{n+1} = \infinitesimal{t} f_{n+1} + u_n
        \]
        \[
          \integral{\domain}{}{u_{n+1}w}{λ}
