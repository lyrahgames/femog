...
\begin{document}
  ...
  \label{sec:methoden}
    ...
    \label{sub:finite_elemente_methode}
      ...
      Um ein Problem zu lösen, unterteilt es das Berechnungsgebiet in kleinere und einfach zu behandelnde Teile, die auch finite Elemente genannt werden.
      ...
      \subsubsection{Konstruktion der schwachen Formulierung}
        ...
        Allerdings wurde im Abschnitt \ref{ssub:wellengleichung} bereits erwähnt, dass die für die Finite-Elemente-Methode benötigte schwache Formulierung der Wellengleichung durch die separate Diskretisierung der Zeit erlangt wird \cite{Alberty1998,Logan2007}.
        ...
        \hfill\\
          ...
          Alternativ ließen sich auch andere Zeitschrittverfahren verwenden.
          ...
          \[
            ...
            \end{pmatrix}
          \]
          Des Weiteren gelten die folgenden für die Dirichlet-Randbedingungen wichtigen Zusammenhänge für alle $t\in T\define[0,\infty)$.
          \[
            φ(\cdot,0) = φ_0
            \separate
            ϑ(\cdot,0) = ϑ_0
          ...
          \]
          Es soll nun ein kleiner Zeitschritt $\infinitesimal{t}\in(0,\infty)$ gewählt werden, der das Zeitintervall $T$ durch $\boxBrackets{T}\define\set{k\cdot\infinitesimal{t}}{k\in\setNatural_0}$ diskretisiert.
          Weiterhin bezeichne eine natürliche Zahl $n\in\setNatural$ als Index der zeitabhängigen Funktionen $f$, $φ^{(\mathrm{D})}$, $φ^{(\mathrm{N})}$, $φ$ und $ϑ$ deren Diskretisierung zum Zeitpunkt $n\cdot\infinitesimal{t}$.
          Dieses Schema soll im Folgenden am Beispiel von $φ$ demonstriert werden.
          ...
          \[
            ...
            \end{pmatrix}
          \]
          Die Konstruktion der schwachen Formulierung kann nun durch dieselbe Vorgehensweise wie bei der Poisson-Gleichung erreicht werden.
          ...
          \[
            \integral{\domain}{}{ϑ^\circ_{n+1}ξ}{λ} = \integral{\domain}{}{ϑ^\circ_{n}ξ}{λ} - \infinitesimal{t} \integral{\domain}{}{\scalarProduct{\nabla φ^\circ_n}{\nabla ξ}}{λ}
          \]
      ...
      \label{ssub:discretization-domain}
        ...
        Das weitere Vorgehen der nächsten Paragraphen wird am Beispiel des eindimensionalen Berechnungsgebiet aus Abbildung \ref{fig:domain-example} gezeigt.
        ...
        Wichtig dabei ist, dass sich nur die Kanten der Dreiecke schneiden dürfen, nicht aber die Dreiecke selbst, da sonst ein Teil des Berechnungsgebietes durch mehr als ein Dreieck beschrieben werden würde.
      ...
      \label{ssub:system-matrix-construction}
        ...
        \hfill\\
          ...
          \[
            [\laplacian][v] = [M][f] + [N]\boxBrackets{u^\mathrm{(N)}} - [\laplacian]\boxBrackets{u^\mathrm{(D)}}
          \]
          Bei $[\laplacian]$ handelt es sich um eine symmetrische, positiv-definite Matrix \cite{Alberty1998}.
          Für das System linearer Gleichungen existiert damit eine eindeutige Lösung \cite{Alberty1998,Schweizer2013}.

          \paragraph{Wellenproblem}
          \hfill\\
            Für die Wellengleichung wird ein analoges Verfahren verwendet.
            Es sollen jedoch die Dirichlet-Randpunkte von den restlichen Punkten separiert werden.
            Es sei also $(\boxBrackets{\Omega},f,u_0,u_1,u_\mathrm{D}, u_\mathrm{N},u)$ ein Wellenproblem.
            Dann lässt sich ein Matrix-Vektor-Produkt in der folgenden Form schreiben.
            \[
              [M][ϑ_n] \reverseDefine \underbrace{[M]^\circ [ϑ_n]^\circ}_{\text{innere Eckpunkte}} + \underbrace{[M]^\mathrm{(D)}[ϑ_n]^\mathrm{(D)}}_{\text{Dirichlet-Eckpunkte}}
            \]
            Dieses Verfahren wird nun auf die schwache Formulierung der zeit-diskretisierten Wellengleichung angewendet, wodurch man das folgende lineare Gleichungssystem erhält.
            Auffallend ist, dass in der Finite-Elemente-Formulierung der Wellengleichung dieselben Matrizen, wie in der Finite-Elemente-Formulierung der Poisson-Gleichung vorkommen.
            \begin{align*}
              [M]^\circ [ϑ_{n+1}]^\circ &= [M]^\circ [ϑ_n]^\circ + [M]^\mathrm{(D)}[ϑ_n]^\mathrm{(D)} \\
              &- \frac{1}{\infinitesimal{t}} \roundBrackets{
                [M]^\mathrm{(D)}[φ_{n+1}]^\mathrm{(D)} - [M]^\mathrm{(D)}[φ_n]^\mathrm{(D)}
              } \\
              &+ \infinitesimal{t}\left(
                [M]^\circ [f_n]^\circ + [M]^\mathrm{(D)}[f_n]^\mathrm{(D)} + [N][φ_n]^\mathrm{(N)} \right. \\
              &\left. - [\laplacian]^\circ [φ_n]^\circ - [\laplacian]^\mathrm{(D)}[φ_n]^\mathrm{(D)} \right)
            \end{align*}

          \paragraph{Finite-Elemente-Matrizen}
          \hfill\\
        ...
        \hfill\\
          ...
          \[
            ...
            \end{pmatrix}
          \]
          Aus der Addition der einzelnen Finite-Elemente-Matrizen ergeben sich dann die kompletten System-Matrizen, auch \enquote{Mass Matrix} und \enquote{Stiffness Matrix} genannt.
          In den folgenden Matrizen wurden Einträge mit dem Wert Null nicht explizit notiert.
          \[
          ...
          Die \enquote{Stiffness Matrix} ist symmetrisch und positiv definit.
          Das System besitzt damit eine eindeutige Lösung.
          {\small
          \[
            \frac{1}{h}
            \begin{pmatrix}
        ...
        \hfill\\
          Für die Berechnung der System-Matrizen eines zweidimensionalen Berechnungsgebietes ist ein analoges Vorgehen möglich.
          Die Integrale des vorigen Abschnittes müssen auch hier über den finiten Elementen ausgewertet und zu einer Systemmatrix zusammengesetzt werden.
          Für die Implementierung des Algorithmus reicht es demnach, die Finite-Elemente-Matrizen anzugeben.
          Die Berechnung dieser soll aufgrund des Umfanges ihrer Herleitungen nicht angegeben werden.
          \[
      ...
      \subsubsection{Lösung des algebraischen Systems}
        Die erhaltenen linearen Gleichungssysteme sind äquivalent zu der schwachen Formulierung des diskretisierten Problems und werden durch dünnbesetzte Matrizen charakterisiert.
        Zudem sind die hier konstruierten inneren Systemmatrizen symmetrisch und positiv definit.
        Daraus folgt, dass für die linearen Gleichungssysteme eine eindeutige Lösung existiert \cite{Schweizer2013,Alberty1998}.
        Für die Lösung dieser Systeme ist eine Vielzahl an unterschiedlichen Methoden, wie zum Beispiel das CG-Verfahren, welches in Abschnitt \ref{ssub:cg-method} erläutert wird, dokumentiert.
    ...
    \label{sub:sparse_matrix_methoden}
      ...
      \subsubsection{Lösung dünnbesetzter Systeme und das CG-Verfahren} % (fold)
      \label{ssub:cg-method}
        Das CG-Verfahren (engl.: \textit{conjugate gradient method}) ist eine iterative Methode, um das folgende lineare Gleichungssystem zu lösen, welches durch eine symmetrische, positiv definite Matrix $A\in\setReal^{n\times n}$, einen Vektor $b\in\setReal^n$ und eine eindeutige Lösung $x\in\setReal^n$ für $n\in\setNatural$ beschrieben wird.
        \[
          Ax = b
        \]
        Eine äquivalente Formulierung ist dabei durch das folgende Optimierungsproblem zusammen mit dessen Residuum gegeben.
        \[
          \min_{x\in\setReal^n} φ(x) \define \transpose{x}Ax - \transpose{b}x
          \separate
          \nabla φ(x) = Ax-b \reverseDefine r(x)
        \]
        Das CG-Verfahren konvergiert nach spätestens $n$ Schritten zur korrekten Lösung.
        Insbesondere nimmt mit jeder Iteration der Gesamtfehler der approximierten Lösung ab, sodass das Verfahren bereits nach einer geringeren Anzahl an Iterationen abgebrochen werden kann.

        Grundsätzlich basiert das Verfahren auf der Konstruktion von $n$ zueinander paarweise $A$-konjugierten Vektoren $p_i$ für $i\in\setNatural$ mit $i\leq n$.
        Auf der Grundlage dieser Vektoren kann dann in jeder Iteration ein neuer Lösungsvektor konstruiert werden, dessen Gesamtfehler reduziert wurde.
        Der folgende Algorithmus wurde \cite{Nocedal2006} entnommen und beschreibt das Verfahren zur Berechnung des Lösungsvektors $x$, sofern ein beliebiger Startwert $x_0\in\setReal^n$ gegeben ist.
        Für eine detailliertere Behandlung des CG-Verfahrens sei hier auf \cite{Nocedal2006} verwiesen.
        Dort wird auch die Möglichkeit des sogenannten Vorkonditionierens beschrieben, die die Konvergenzgeschwindigkeit des Verfahrens noch erhöht.
        \cite{Nocedal2006}

        In Abschnitt \ref{sub:time-step-computation} wurde das hier beschriebene CG-Verfahren in der Sprache C++ auf der CPU und auf der GPU unter Zuhilfenahme der Sprache CUDA, der Bibliothek \enquote{Eigen} und der Bibliothek \enquote{Thrust} implementiert \cite{cuda2018,Eigen2018}.
        ...
        \begin{center}
          \begin{tcolorbox}[%
            ...
            breakable,
            width=0.7\textwidth,
            title=Algorithmus:\quad CG-Verfahren
          ]
            \begin{align*}
              &\text{\textbf{\color{MidnightBlue}Given: }} A\in\setReal^{n\times n},x_0\in\setReal^n,b\in\setReal^n \\
              &r_0 \leftarrow Ax_0 - b \\
              &p_0 \leftarrow -r_0 \\
              &k \leftarrow 0 \\
              &\text{\textbf{\color{MidnightBlue}while} } r_k\neq 0 \\
              &\qquad α_k \leftarrow \frac{\transpose{r_k}r_k}{\transpose{p_k}Ap_k} \\
              &\qquad x_{k+1} \leftarrow x_k + α_kp_k \\
              &\qquad r_{k+1} \leftarrow r_k + α_kAp_k \\
              &\qquad β_{k+1} \leftarrow \frac{\transpose{r_{k+1}}r_{k+1}}{\transpose{r_k}r_k} \\
              &\qquad p_{k+1} \leftarrow -r_{k+1} + β_{k+1}p_k \\
              &\qquad k \leftarrow k+1 \\
              &\text{\textbf{\color{MidnightBlue}end}}
            \end{align*}
          \end{tcolorbox}
        \end{center}
